...
func Decode(b []byte) (File, error) {
	r := bytes.NewReader(b)
	var f File
	if e := binary.Read(r, binary.LittleEndian, &f.Header); e != nil {
		return f, e
	}
	do := func(a, b error) error {
		if a != nil {
			return a
		}
		return b
	...
	f.alloc()
	var e error
	e = do(e, binary.Read(r, binary.LittleEndian, f.Time))
	e = do(e, binary.Read(r, binary.LittleEndian, f.Dist))
	e = do(e, binary.Read(r, binary.LittleEndian, f.Alt))
	e = do(e, binary.Read(r, binary.LittleEndian, f.Lat))
	e = do(e, binary.Read(r, binary.LittleEndian, f.Lon))
	return f, e
}
